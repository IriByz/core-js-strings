...
function removeFirstOccurrences(str, value) {
  let indOf = str.indexOf(value);
  let valLen = value.length;
  valLen = indOf < 0 ? 0 : valLen;
  indOf = indOf < 0 ? 0 : indOf;
  return str.substring(0, indOf).concat(str.substring(indOf + valLen));
}
...
function removeLastOccurrences(str, value) {
  let indOf = str.lastIndexOf(value);
  let valLen = value.length;
  valLen = indOf < 0 ? 0 : valLen;
  indOf = indOf < 0 ? 0 : indOf;
  return str.substring(0, indOf).concat(str.substring(indOf + valLen));
...
function isPalindrome(str) {
  let strNoSpace = str.replaceAll(' ', '').toLowerCase();
  strNoSpace = strNoSpace.replaceAll('?', '').replaceAll('!', '');
  strNoSpace = strNoSpace.replaceAll(',', '');
  return reverseString(strNoSpace) === strNoSpace;
}
